...
import {SexEnum, IsYetEnum, MsgEnum} from '@/enums'
...
const options = reactive({ multiple: false, accept: '.jpg,.png,.gif,.jpeg,.webp' })
...
const selectAndUploadFile = async (files?: FileList | null) => {
  ...
  useUploadChange((status) => {
    if (status === 'success') {
      ...
      // 更改头像
      apis.modifyUserAvatar(fileInfo.value.downloadUrl).send().then(s => {
        userStore.getUserDetailAction()
      })
    }
    reset()
  ...
  >
    <div class="setting-box">
      <div class="setting-avatar-box">
        <el-button @click="openFileSelect">
          <ElAvatar
              size="large"
