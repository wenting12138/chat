...
public abstract class AbstractUrlDiscover implements UrlDiscover {
    ...
    public Map<String, UrlInfo> getUrlContentMap(String content) {
        ...
        //结果组装
        return future.join().stream().collect(Collectors.toMap(Pair::getObject1, Pair::getObject2, (a, b) -> a));
    }
    ...
    public UrlInfo getContent(String url) {
        ...
        urlInfo.setTitle(getTitle(document));
        urlInfo.setDescription(getDescription(document));
        urlInfo.setImage(getImage(assemble(url), document));
        return urlInfo;
    }
